...
struct Camera
{
    ...
    float windowWidth = 1280.0f;  // Window width for rendering
    float windowHeight = 720.0f; // Window height for rendering

    float zoom = 1.0f;            // Zoom level of the camera
    float worldScale = 1.0f;      // Scaling factor for the world

    unsigned int displayWidth = 1280;  // Display width
    unsigned int displayHeight = 720; // Display height
};
...
static void Resize(const int& width, const int& height)
{
    ...
    float displayScale = fmin(displayScaleX, displayScaleY) * g_Camera.worldScale;
    
    // Log the calculated scale factor
    dmLogInfo("Scale: %f", displayScale);

    // Set the position of the camera (shifted to center the view)
    dmGameObject::SetPosition(g_Camera.mainCam, Point3(width * -.5f, height * -.5f, 0.0f));

    // Apply the calculated scale to the world target
    dmGameObject::SetScale(g_Camera.worldTarget, Vector3(displayScale, displayScale, displayScale));
}
...
static int InitCamera(lua_State* L)
{
    ...
    g_Camera.worldScale = dmGameObject::GetScale(world).getX();
...
{
	g_Camera.displayWidth = dmConfigFile::GetInt(params->m_ConfigFile, "display.width", 1280);
	g_Camera.displayHeight = dmConfigFile::GetInt(params->m_ConfigFile, "display.height", 720);
